var cloudaclWebFilter = (function () {
    "use strict";
    ...
    return {
        ...
        */
        ...
        contentListener: function (aEvent) {
            ...
            cloudaclWebFilter.processNewURL(aURI);
        },
        WebBlockListener: {
        ...
        init: function () {
            ...
            this.readUrlDB();
            this.readTopDomain();
            this.strings = document.getElementById("cloudaclWebFilter-strings");
            this.checkVersionChanged();
            cloudaclWebFilter.WebBlockPrefObserver.register();
            document.addEventListener("DOMContentLoaded", cloudaclWebFilter.contentListener, false);
            gBrowser.addProgressListener(cloudaclWebFilter.WebBlockListener);
        },
        uninit: function () {
            if (this.getBoolPref("bg_verified")) {
                this.setBoolPref("bg_verified", false);
            }
            gBrowser.removeProgressListener(cloudaclWebFilter.WebBlockListener);
        },
        log: function (s) {
        ...
        processNewURL: function (aURI) {
            ...
            if (aURI === null) { return; }
            var url = aURI.spec;
            var p = new cloudaclWebFilter.URLParser(url);
            var proto = p.getProtocol();
            if (proto != "http" && proto != "https" && proto != "ftp") { return; }
            var host = cloudaclWebFilter.urlNormorlize(p.getHost());
            if (/google\./.test(host)) {
                if (/\/url\?/.test(url)) {
                    return;
                }
            ...
            if (cid === 100 || this.isCategoryBlocked(cid)) {
                ...
                cloudaclWebFilter.queryCloud(url2, isIP, url);
            }
        },
            ...
            req.onreadystatechange = function () {
                if (req.readyState === 4) {
                    ...
                    if (cloudaclWebFilter.isCategoryBlocked(cid)) {
                        cloudaclWebFilter.blockBadHost(host, cid);
                    }
                }
            };
        ...
        isCategoryBlocked: function (cid) {
            ...
            if (cloudaclWebFilter.urlPolicy.containsKey(cid)) {
                var isBlocked = cloudaclWebFilter.urlPolicy.get(cid);
        ...
        WebBlockPrefObserver: {
            ...
            observe: function (aSubject, aTopic, aData) {
                ...
                if (/cid.+/i.test(aData)) {
                    cloudaclWebFilter.urlCache.clear();

                    switch (aData) {
                        case "cid1pref":
                            cloudaclWebFilter.urlPolicy.put("1", cloudaclWebFilter.urlPolicy.get("1") ? false : true);
                            break;
                        case "cid2pref":
                            cloudaclWebFilter.urlPolicy.put("2", cloudaclWebFilter.urlPolicy.get("2") ? false : true);
                            break;
                        case "cid3pref":
                            cloudaclWebFilter.urlPolicy.put("3", cloudaclWebFilter.urlPolicy.get("3") ? false : true);
                            break;
                        case "cid4pref":
                            cloudaclWebFilter.urlPolicy.put("4", cloudaclWebFilter.urlPolicy.get("4") ? false : true);
                            break;
                        case "cid5pref":
                            cloudaclWebFilter.urlPolicy.put("5", cloudaclWebFilter.urlPolicy.get("5") ? false : true);
                            break;
                        case "cid6pref":
                            cloudaclWebFilter.urlPolicy.put("6", cloudaclWebFilter.urlPolicy.get("6") ? false : true);
                            break;
                        case "cid7pref":
                            cloudaclWebFilter.urlPolicy.put("7", cloudaclWebFilter.urlPolicy.get("7") ? false : true);
                            break;
                        case "cid8pref":
                            cloudaclWebFilter.urlPolicy.put("8", cloudaclWebFilter.urlPolicy.get("8") ? false : true);
                            break;
                        case "cid9pref":
                            cloudaclWebFilter.urlPolicy.put("9", cloudaclWebFilter.urlPolicy.get("9") ? false : true);
                            break;
                    }
        ...
        readUrlDB: function () {
            var s = this.readfile("chrome://cloudaclWebFilter/content/category.json");
            if (s) {
                var d = JSON.parse(s);
                for (var n in d) {
        ...
        readTopDomain: function () {
            var s = this.readfile("chrome://cloudaclWebFilter/content/top_domain.json");
            if (s) {
                var d = JSON.parse(s);
                for (var n in d) {
        ...
        checkVersionChanged: function () {
            ...
            this.show_toolbar_button("cloudaclWebFilter-toolbar-button", "urlbar-container");
        ...
        onMenuItemCommand_tooloption: function (e) {
            var tab = getBrowser().addTab("chrome://cloudaclWebFilter/content/options.html");
        ...
        updateToolbarMenuLabel: function (event) {
            var mb = document.getElementById("context-cloudaclWebFilter-addBlock");
            var mt = document.getElementById("context-cloudaclWebFilter-addTrust");
            var mc = document.getElementById("context-cloudaclWebFilter-clearSite");
